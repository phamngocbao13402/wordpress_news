...
abstract class WC_Base_Mpay extends WC_Payment_Gateway
{
	...
	public function __construct()
	{
		...
		if (true || !isset($wp_session['mpay_banks_setting'])) {
			$this->plugin_settings = MpayPayment::get_settings();
			$this->oauth_settings = MpayPayment::oauth_get_settings();
			#$wp_session['mpay_banks_setting'] = $this->plugin_settings;
		} else {
			//$this->plugin_settings = $wp_session['mpay_banks_setting'];
		}
	...
	public function init_form_fields()
	{
		$this->form_fields = array(
			...
			'title'           => array(
				...
				'default'     => sprintf(__('Transfer %s', 'bck-verify-bank-transfer'),$this->bank_name),
				'desc_tip'    => true,
			),
			'description'     => array(
				...
				'default'     => sprintf(__("Transfer money to our account<b> %s</b>. The order will be confirmed immediately after the transfer", 'bck-verify-bank-transfer'), $this->bank_name),
...
<?php
	...
	public function save_account_details() {
		...
		if(!empty($accounts)) {
			...
			MpayPayment::update_settings($this->plugin_settings);
	...
	public function thankyou_page($order_id)
	{
		...
		#hpc_console_log($this->account_details);
		global $wp_session;
...
if($is_payment) {
	$output .= '<div class="mpay-success"><img src="' . MPAY_URL.'/assets/success-icon.png' . '"  style = "max-width: 100px; margin: 20px" id =""/>';
	...
	$payment_gateways = WC()->payment_gateways->payment_gateways();
			foreach ($bacs_accounts as $i=> $bacs_account) {
				...
				$account_fields = apply_filters(
					'woocommerce_hpc_account_fields',
					array(
						'bank_name'      => array(
							'label' => __('Bank', 'bck-verify-bank-transfer'),
							'value' => !empty($payment_gateways['mpay_up_'.$bacs_account->bank_name])? $payment_gateways['mpay_up_'.$bacs_account->bank_name]->bank_name: strtoupper($bacs_account->bank_name),
						),
						'account_number' => array(
							'label' => __('Account number', 'bck-verify-bank-transfer'),
							'value' => $bacs_account->account_number,
						),
						'account_name' => array(
							'label' => __('Account name', 'bck-verify-bank-transfer'),
							'value' => $bacs_account->account_name,
						),
						'bin' => array(
							'label' => __('Bin', 'bck-verify-bank-transfer'),
							'value' => $bacs_account->bin,
						),
						'amount'            => array(
							'label' => __('Amount', 'bck-verify-bank-transfer'),
							'value' => number_format($order->get_total(), 0),
						),
						'content'            => array(
							'label' => __('Content', 'bck-verify-bank-transfer'),
							'value' =>	MpayPayment::transaction_text($this->plugin_settings['bank_transfer']['transaction_prefix'] . '' . $order_id, $this->plugin_settings),
						),
					),
					$order_id
				);
				...
				if (!$is_payment) {
					...
					if (empty($this->order_content)) {
						$order_content = __('I have already paid', 'bck-verify-bank-transfer');
					} else {
						$order_content = $this->order_content;
					}
				} else {
					$order_content = __('You paid', 'bck-verify-bank-transfer');
					$disabled .= 'disabled';
				}
				$banks_list = MpayPayment::get_list_banks();
				...
				if ($has_details) {
					...
					if (!$is_payment && $qrcode_url) {	//$this->plugin_settings['bank_transfer']['viet_qr'] == 'yes' && 
						$showPayment .= '
					<section class="woocommerce-mpay-bck-qr-scan">
						'.sprintf('<img src="%s" class="bank-logo" style="%s"/>',MpayPayment::get_bank_icon($bacs_account->bank_name), MpayPayment::noQRBankLogo($bacs_account->bank_name)? 'display:none':'display:inline-block').'
					<h2 class="wc-mpay-bck-bank-details-heading" style="text-align: center; margin-top: 20px">' . __('Bank transfer QR code', 'bck-verify-bank-transfer')  . '</h2>

					<div style="">
						<div id="qrcode" style="text-align: center;">
							<img src="' . esc_html($qrcode_url) . '" onerror="qrcode_fallback(this)"  alt="mpay-bck QR Image" width="400px" />
						</div>
						...
						$showPayment .= sprintf('<img src="%s" class="bank-logo"/>',MpayPayment::get_bank_icon($bacs_account->bank_name));
					}
					$showPayment .= '<section class="woocommerce-mpay-bck-bank-details">
						<!-- BANK DETAIL TITLE-->
						<h2 class="wc-mpay-bck-bank-details-heading" style="text-align: center;">' . esc_html__('Our bank details', 'bck-verify-bank-transfer') . '</h2>';
					if (!$is_payment)
						$showPayment .= '<div><h4 style="color: #856404; max-width: 750px; margin: auto; margin-bottom: 20px; background-color: #ffeeba; padding: 15px; border-radius: 7px;">' .  sprintf(__("Please transfer the correct content <b style='font-size: 20px;'>%s</b> for we can confirm the payment", 'bck-verify-bank-transfer'), esc_html($account_fields['content']['value'])) . '</h4></div>
					...
					$showPayment .= '
						...
						<tr class="" >
								<td class="text-right"  style="text-align: right;">
									<strong style="color: black;">' . __('Account name', 'bck-verify-bank-transfer') . ':</strong>
									<br>
								</td>
							...
							<tr class="" style="background-color:#FBFBFB;">
								<td class="text-right"  style="text-align: right;">
									<strong style="color: black;">' . __('Account number', 'bck-verify-bank-transfer') . ':</strong>
								</td>
								<td class="text-left payment-instruction" style="text-align: left;">
							...
							<tr class="" style="">
								<td class="text-right" style="text-align: right;">
									<strong style="color: black;">' . __('Bank', 'bck-verify-bank-transfer') . ':</strong>
									<br>
								</td>
								<td class="text-left payment-instruction" style="text-align: left;">
									<div>
							...
							<tr class="" style="">
								<td class="text-right"  style="text-align: right;">
									<strong style="color: black;">' . __('Amount', 'bck-verify-bank-transfer') . ':</strong>
									<br>
								</td>
								<td class="text-left payment-instruction" style="text-align: left;">
							...
							<tr class="" >
								<td class="text-right" style="text-align: right;">
									<strong style="color: black;">' . __('Content', 'bck-verify-bank-transfer') . '*:</strong>
								</td>
								<td class="text-left payment-instruction" style="text-align: left;">
						...
						<button  name="submit_paid" id="input_mpay" style="margin-bottom: 20px;" '.($disabled? '': 'onclick="fetchStatus('.$i.')"').' type="button"
						class="button '.$disabled.'"  >' . $order_content  . '</button>
						</form>
						</center>
						<h5 style="color: red; display: none;" id="noTransaction_'.$i.'">' .  __("No matching transfers were found. The system is still checking the transaction", 'bck-verify-bank-transfer') . '</h5>
					</section>
					...
					$showPayment.= '<script>
						function fetchStatus(i)
						{
							...
							setInterval(function(){
								jQuery.ajax({
									...
									type : "post",    
									data: {action: "fetch_order_status_hpc", order_id: '.$order_id.'},  
						...
						function qrcode_fallback(e) {
							...
							jQuery(e).closest(".woocommerce-mpay-bck-qr-scan").find(".wc-mpay-bck-bank-details-heading").hide();
			...
			$output .= '</div>';
			echo wp_kses_post($output);//echo $output;
		}
	...
	public function get_qrcode_vietqr_img_url($account_fields)
	{
		...
		if(is_numeric($acqId)) {
			...
			$pay_url = "";
			if($bank=='momo') {
				$img_url = get_rest_url(null, "bck/v1/qrcode?app=momo&phone={$accountNo}&price={$amount}");
			}
			else if($bank=='viettelpay') {
				$img_url = get_rest_url(null, "bck/v1/qrcode?app=viettelpay&phone={$accountNo}&price={$amount}&content=".urlencode($addInfo));
			}
		}
		return array(
			"img_url" => $img_url,
	...
	public function get_description()
	{
		...
		#	$des .= __(" <div class='power_by'>Power by PaidChecker</div>", 'bck-verify-bank-transfer');
		return $des;
	}
	public static function payment_name($name) {
		return str_replace('mpay_up_', '', $name);
	}
}
